...
The data used in this section has been downloaded from [GEO](https://www.ncbi.nlm.nih.gov/geo/) (accession number GSE66351) which contains DNA methylation profiling (Illumina 450K array). Data corresponds to CpGs beta values measured in the superior temporal gyrus and prefrontal cortex brain regions of patients with Alzheimer's. The data is encapsulated on `ExpressionSet` objects. Researchers who are not familiar with `ExpressionSet` can find information [here](https://www.bioconductor.org/packages/release/bioc/vignettes/Biobase/inst/doc/ExpressionSetIntroduction.pdf). Notice that data is encoded as beta-values that ensure data harmonization across studies.
...
- Full ExpressionSet meta-analysis
- Full ExpressionSet meta-analysis adjusting for surrogate variables
